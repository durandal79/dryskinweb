    ...
    io.on("connection", function (socket) {
        ...
        request('http://freegeoip.net/json/', function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                var geoip = JSON.parse(body); // parse weather data

                var geodat ={city:geoip.city, region: geoip.region_code, lat:geoip.latitude, long: geoip.longitude};

                io.emit("geoout", geodat);

                var lat = geoip.latitude;
                var long = geoip.longitude;
